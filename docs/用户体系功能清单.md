...
> 说明（与本项目现状的映射）：
> - 本项目后台用户/角色/权限主要落在 `hs_admin_user`（`AdUser/AdRole/AdPermission/...`）与 `hs_api` 的 `/admin/**` 接口中；
> - 数据表命名在不同环境可能存在差异（如 `admin_user` vs `ad_user`、`adPermission` vs `ad_permission`），本清单以“能力是否落地”为准。
...
- [x] 设计并创建基础表（或已有等价表）：
  - [x] 用户表（等价 `sys_user`）
  - [x] 角色表（等价 `sys_role`）
  - [x] 权限表（等价 `sys_permission`）
  - [x] 用户-角色关联表（等价 `sys_user_role`）
  - [x] 角色-权限关联表（等价 `sys_role_permission`）
...
- [ ] 初始化数据（seed）：
  - [ ] 内置角色：`ADMIN`（当前内置角色多为 `SUPER_ADMIN/SYSTEM_ADMIN/USER`，建议补齐 `ADMIN` 或统一策略）
  - [ ] 内置权限点：覆盖本清单涉及的接口（见第 7 节）
  - [ ] 初始化管理员账号（首次启动可用）

> 本项目已提供 seed 脚本（执行后可将上述两项改为 `[x]`）：
> - `/Users/tujunjie/Downloads/project/eat/sql/2026_02_27_admin_permission_seed.sql`
> - `/Users/tujunjie/Downloads/project/eat/sql/2026_02_27_admin_user_seed.sql`
...
- [x] 用户详情：基本信息 + 角色
- [x] 新增用户：设置 username、display_name、初始密码、enabled、角色（支持一次性带角色创建）
- [x] 编辑用户：修改 display_name、enabled、角色（支持一次性带角色更新）
