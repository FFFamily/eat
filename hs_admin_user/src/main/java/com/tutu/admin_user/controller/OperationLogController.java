...
import com.tutu.common.annotation.OperationLog as LogAnnotation;
...
public class OperationLogController {
    ...
    public BaseResponse<IPage<OperationLog>> getPageList(
        ...
        IPage<OperationLog> page = operationLogService.getPageList(current, size, userId, operationType, keyword, startTime, endTime);
    ...
    public BaseResponse<OperationLog> getById(@PathVariable String id) {
        OperationLog operationLog = operationLogService.getById(id);
        if (operationLog == null) {
            return BaseResponse.error("操作日志不存在");
        }
        return BaseResponse.success(operationLog);
    }
    ...
    public BaseResponse<List<OperationLog>> findByUserId(
            @PathVariable String userId,
            @RequestParam(defaultValue = "10") int limit) {
        List<OperationLog> logs = operationLogService.findByUserId(userId, limit);
    ...
    public BaseResponse<List<OperationLog>> findByOperationType(
            @PathVariable String operationType,
            @RequestParam(defaultValue = "10") int limit) {
        List<OperationLog> logs = operationLogService.findByOperationType(operationType, limit);
        return BaseResponse.success(logs);
    }
    ...
    public BaseResponse<Map<String, Object>> getStatistics(@RequestParam(required = false) String userId) {
        ...
        // 今日总操作次数
        Long todayTotal = operationLogService.countTodayOperations();
        statistics.put("todayTotal", todayTotal);
        
        // 用户今日操作次数
        if (userId != null) {
            Long userTodayTotal = operationLogService.countUserTodayOperations(userId);
    ...
    public BaseResponse<String> batchDeleteLogs(@RequestBody List<String> ids) {
        try {
            boolean result = operationLogService.batchDeleteLogs(ids);
            if (result) {
