...
public interface AdPermissionMapper extends BaseMapper<AdPermission> {
    ...
    AdPermission findByCode(@Param("code") String code);
    ...
    IPage<AdPermission> selectPageByKeyword(Page<AdPermission> page, @Param("keyword") String keyword);
    ...
    List<AdPermission> findByRoleId(@Param("roleId") String roleId);
    ...
    @Select("SELECT DISTINCT p.* FROM permission p " +
            ...
            "ORDER BY p.sort_order ASC")
    List<AdPermission> findByUserId(@Param("userId") String userId);
    ...
    List<AdPermission> findByParentId(@Param("parentId") String parentId);
    ...
    @Select("SELECT * FROM permission WHERE type = 1 AND is_deleted = '0' " +
            "ORDER BY sort_order ASC")
    List<AdPermission> findAllMenus();
}
