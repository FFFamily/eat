...
public interface AdRolePermissionMapper extends BaseMapper<AdRolePermission> {
    ...
    List<AdRolePermission> findByRoleId(@Param("roleId") String roleId);
    
    /**
     * 根据权限ID查询角色权限关联
     */
    @Select("SELECT * FROM role_permission WHERE permission_id = #{permissionId} AND is_deleted = '0'")
    List<AdRolePermission> findByPermissionId(@Param("permissionId") String permissionId);
     ...
     * 批量插入角色权限关联
     */
    int batchInsert(@Param("rolePermissions") List<AdRolePermission> adRolePermissions);
}
