...
public interface AdUserRoleMapper extends BaseMapper<AdUserRole> {
    ...
    List<AdUserRole> findByUserId(@Param("userId") String userId);
    
    /**
     * 根据角色ID查询用户角色关联
     */
    @Select("SELECT * FROM admin_user_role WHERE role_id = #{roleId} AND is_deleted = '0'")
    List<AdUserRole> findByRoleId(@Param("roleId") String roleId);
     ...
     * 批量插入用户角色关联
     */
    int batchInsert(@Param("userRoles") List<AdUserRole> userRoles);
}
